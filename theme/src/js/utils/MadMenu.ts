...
// This script handles WMPotify's custom menu bar and context menus (not the Spotify's React-based one)
...
export class MadMenu {
    ...
    closeMenu(menuName) {
        ...
        if (standalone) {
            ...
            parentMenuItem?.removeAttribute('data-active');
        }
    ...
    delayedCloseMenu(menuName: string) {
        this.submenuCloseTimer = setTimeout(() => {
            this.closeMenu(menuName);
        }, 300);
    }

    menuNavigationHandler(event: KeyboardEvent) {
        if (!this.openedMenu) {
            return;
        }
        ...
        const activeItem = this.openedMenu.querySelector<HTMLElement>('.contextMenuItem[data-active]');
        const activeItemIndex = activeItem ? Array.from(menuItems).indexOf(activeItem) : -1;
        let parentMenuItem: HTMLElement | null = null;
        if (this.openedMenu.dataset.submenuOf) {
            parentMenuItem = document.getElementById(`${this.openedMenu.dataset.submenuOf}MenuBg`)?.querySelector<HTMLElement>('.contextMenuItem[data-active]') || null;
        }
        ...
        switch (event.key) {
            ...
            case "ArrowUp":
                if (activeItem && activeItem instanceof HTMLElement) {
                    ...
                    if (activeItemIndex > 0) {
                        (menuItems[activeItemIndex - 1] as HTMLElement).setAttribute('data-active', 'true');
                    } else {
                        (menuItems[menuItems.length - 1] as HTMLElement).setAttribute('data-active', 'true');
                    }
                } else {
                    (menuItems[menuItems.length - 1] as HTMLElement).setAttribute('data-active', 'true');
                }
            ...
            case "ArrowDown":
                if (activeItem && activeItem instanceof HTMLElement) {
                    ...
                    if (activeItemIndex < menuItems.length - 1) {
                        ...
                        (menuItems[0] as HTMLElement).setAttribute('data-active', 'true');
                    }
                } else {
                    (menuItems[0] as HTMLElement).setAttribute('data-active', 'true');
                }
            ...
            case "ArrowLeft": {
                if (this.openedMenu.dataset.submenuOf) {
                    ...
                    parentMenuItem?.setAttribute('data-active', 'true');
                    break;
                }
            ...
            case "ArrowRight": {
                if (activeItem && activeItem instanceof HTMLElement && activeItem.dataset.submenu) {
                    ...
                    openedSubMenuItem.setAttribute('data-active', 'true');
                    break;
                }
                if (this.menuOrder.length === 1) {
                    break;
                ...
                const openedMenuItem = this.openedMenu.querySelector('.contextMenuItem') as HTMLElement;
                openedMenuItem.setAttribute('data-active', 'true');
                break;
            }
            case "Enter":
                if (activeItem && activeItem instanceof HTMLElement) {
    ...
    calcMenuWidth(menuName: string): string {
        ...
        const minWidth = parseInt(getComputedStyle(menuBg).minWidth, 10) || 0;
        ...
        return `calc(${width}px + 4.5em`; // No closing parenthesis because openMenu adds additional width and the closing parenthesis
    }
...
export interface MadMenuItem {
    ...
    args?: string[]; // An array of arguments to be passed to processString
...
function getTextWidth(text: string, font: string = getComputedStyle(document.documentElement).getPropertyValue("--menu-font")) {
    ...
    const context = savedCanvas.getContext("2d");
    if (!context) {
        return 0;
    }
    context.font = font;
